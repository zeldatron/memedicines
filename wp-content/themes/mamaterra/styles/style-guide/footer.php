...
<!--script>window.jQuery || document.write('<script src="style-guide/vendor/jquery.js"><\/script>')</script-->
<script src="style-guide/js/src-min/ace.js"></script>


<script src="style-guide/js/src-min/ext-emmet.js">  </script>
<script src="style-guide/vendor/emmet/emmet.js"></script>


<script src="style-guide/vendor/zero/ZeroClipboard.js"></script>
<script src="style-guide/vendor/newSort/jquery.fn.sortable.js"></script>
<script src="style-guide/vendor/output-tab.js"></script>
...
<?php if (!empty($_GET['cat']) || !empty($_GET['search'])) { ?>
    ...
    foreach ($compSelect as $component) {
        ...
        <script>
            var editoroverride_<?php echo $i; ?> = ace.edit("editor-override-<?php echo $component['component'] ?>");
            var code = editoroverride_<?php echo $i; ?>.getValue();

            var Emmet = require("ace/ext/emmet");

            editoroverride_<?php echo $i; ?>.setOption("enableEmmet", true);

            $("input[name=new-override-val-<?php echo $component['component'] ?>]").val(code);
            editoroverride_<?php echo $i; ?>.getSession().setUseWorker(false);
            editoroverride_<?php echo $i; ?>.getSession().on('change', function () {
                $("input[name=new-override-val-<?php echo $component['component'] ?>]").val(editoroverride_<?php echo $i; ?>.getSession().getValue());
            });
            var code = code.replace(/<!--(.*?)-->/g, '');
            var code = code.trim();
            $('#<?php echo $component['component'] ?>-container').find(".copyBtn-override").attr('data-clipboard-text', code);
            new ZeroClipboard($('.copyBtn-override'));
            editoroverride_<?php echo $i; ?>.getSession().setMode("ace/mode/html");
            editoroverride_<?php echo $i; ?>.setOptions({
                maxLines: Infinity
            });
            editoroverride_<?php echo $i; ?>.setHighlightActiveLine(false);
            editoroverride_<?php echo $i; ?>.setShowPrintMargin(false);
...
<?php
$filename = '../atomic/atomic-head.php';
if (file_exists($filename)) {
    include ("../atomic/atomic-foot.php");
}
?>
...
<!-- <script src="style-guide/js/min/compiled.min.js"></script> -->
<script src="style-guide/js/min/compiled.js"></script>
